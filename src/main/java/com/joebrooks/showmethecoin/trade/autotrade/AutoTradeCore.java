...
public class AutoTradeCore {
    ...
    @Async
    public void execute(List<CandleStoreEntity> candles, StrategyType strategyType) {
        ...
        try {
            ...
            userConfigList.forEach(user -> {
                ...
                // 익절
                        ...
                        && strategyList.stream()
                        .allMatch(st -> st.isProperToSellWithBenefit(candles, tradeInfoEntities))
                        && tradeInfoEntities.stream().noneMatch(TradeInfoEntity::isOrdered)) {
    ...
    private boolean buyCoin(UserConfigEntity user, CandleStoreEntity nowCandle, CoinType coinType,
                            List<TradeInfoEntity> tradeInfoEntities) {
